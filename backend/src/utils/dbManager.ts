...
// Initialize memoryStore default users
const initMemoryStoreUsers = () => {
  try {
    ...
    console.log(`Initialized in-memory store with ${memoryStore.users.length} default demo users.`);
  } catch (error) {
    console.error('Failed to initialize mock users memory store:', error);
  }
};

// Call initialization
initMemoryStoreFoods();
initMemoryStoreUsers();
